...
/**
 * What it takes to undo one optimistic write.
 *
 * Deliberately *not* a snapshot of the caches. Restoring whole query values is
 * correct only while one mutation is in flight at a time, and that is not how
 * the app is used: add two expenses in quick succession, let the first fail,
 * and its "undo" would put back a cache captured before the second existed -
 * silently deleting a row the user had watched appear and which the server was
 * about to accept.
 *
 * So a reversal describes the write rather than the world around it: drop this
 * row, and apply the negation of this event. `applyEvent` is an additive fold,
 * so the negation cancels exactly what was added no matter what else has landed
 * in between, and removal by id is exact because the id was generated by the
 * client before the row ever went out.
 */
...
/** The inverse of a balance event: the same shape with every amount negated. */
function negate(event: BalanceEvent): BalanceEvent {
  if (event.kind === "expense") {
    return {
      ...event,
      paid: event.paid.map((p) => ({ ...p, amount: -p.amount })),
      owed: event.owed.map((o) => ({ ...o, amount: -o.amount })),
    };
  }
...
/**
 * Undoes one optimistic write, leaving every other one alone.
 *
 * Runs the same cache edits as `applyOptimisticWrite` with the event negated
 * and the row removed instead of inserted, so two failing mutations can be
 * reversed in any order and a mutation that is still in flight keeps its row.
 */
export function revertOptimisticWrite(client: QueryClient, reversal: Reversal | undefined) {
  if (!reversal) return;
  writeOptimistic(client, {
    groupId: reversal.groupId,
    currency: reversal.currency,
    event: negate(reversal.event),
    meId: reversal.meId,
    ledger: (data) => removeFromLedger(data, reversal.entryId),
  });
}

/** Drops a row from every page of a paged ledger. */
function removeFromLedger(
  data: { pages: { items: LedgerEntry[]; nextCursor: string | null }[]; pageParams: unknown[] },
  entryId: string,
) {
  return {
...
export function applyOptimisticWrite(
  ...
  writeOptimistic(client, {
    ...
    event,
    meId,
    ledger: (data) => insertIntoLedger(data, entry),
  });

  return { groupId, currency, entryId: entry.id, event, meId };
}

/**
 * The cache edits shared by writing an optimistic row and taking one back.
 *
 * Both do the same three things - patch the group ledger, fold the event into
 * the group's balance sheet, and move the viewer's dashboard totals - and they
 * have to agree exactly, or an undo leaves a residue. Sharing the code is what
...
function writeOptimistic(
  client: QueryClient,
  {
    ...
    ledger: (data: {
      ...
      pageParams: unknown[];
    }) => { pages: { items: LedgerEntry[]; nextCursor: string | null }[]; pageParams: unknown[] };
  },
) {
  if (groupId) {
    client.setQueriesData<{
      pages: { items: LedgerEntry[]; nextCursor: string | null }[];
      pageParams: unknown[];
    }>({ queryKey: ["group", groupId, "ledger"] }, (data) => (data ? ledger(data) : data));
